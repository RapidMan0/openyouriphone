...
class Calculator {
    ...
    // Метод для вычисления выражений (включая поддержку квадратного корня)
    calculateResult() {
        try {
            ...
            this.display.innerText = eval(expression); // Вычисляем выражение с помощью eval
    ...
    // Обновление дисплея: добавление цифр и операторов
    updateDisplay(scriere) {
        ...
        if (this.lastInputWasResult) {
            ...
            if (this.display.innerText === "0" && scriere !== ".") {
                ...
                this.display.innerText += scriere; //если на экране уже есть какие-то числа, символы или операторы, новый ввод просто добавляется в конец строки
            }
        }
    }
...
// Создаём объект калькулятора для инициализации работы
new Calculator();
